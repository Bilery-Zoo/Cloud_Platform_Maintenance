...
def switcher(cloud_platform, modify_mode, old_whitelistip, key,
    ...
    :param key: AWS / AlibabaCloud access key.
    ...
    if user_input != user_auth:
        ...
        if cloud_platform == "AWS":
            if modify_mode == "append":
                aws_switcher.new_whitelistip_append(old_whitelistip, new_whitelistip, key,
                                                    region=region, is_region_distinguish=is_region_distinguish)
            elif modify_mode == "delete":
                aws_switcher.old_whitelistip_delete(old_whitelistip, key,
                                                    region=region, is_region_distinguish=is_region_distinguish)

        elif cloud_platform == "AlibabaCloud":
            try:
                if modify_mode == "append":
                    alibabacloud_switcher.new_whitelistip_append(old_whitelistip, new_whitelistip, comment, key,
                                                                 region=region, is_region_distinguish=is_region_distinguish)
                elif modify_mode == "delete":
                    alibabacloud_switcher.old_whitelistip_delete(old_whitelistip, key,
                                                                 region=region, is_region_distinguish=is_region_distinguish)
